...
3. **편집** (`/projects/[id]/edit`): 진입 시 인물·장면 그림을 자동 생성·적용한 뒤 원문 | 쉬운 자료 | 문장 도구의 3단 편집기
...
편집 진입 시 서버의 `POST /api/studio/projects/{id}/document/prepare-images`를 완료까지 이어 호출해요.
등장인물 초상을 먼저 만들고 고정한 다음 그 참조로 장면을 만들며, 사용자가 그림을 하나씩 생성·선택할 필요가 없어요.
진행률은 서버의 완료 개수를 표시하고, 페이지를 떠났다가 다시 들어오면 완료된 그림은 유지하고 이어서 진행해요.
등장인물 기준 그림의 변경·삭제는 막고 글·장면 그림·대체텍스트 편집은 유지해요. 초안 재생성도 인물 그림은 유지해요.
실패·접수 불확실은 자동 유료 재시도하지 않아요. 데모/그림 없음은 생성을 건너뛰어요.
